...
type SidebarProps = {
  ...
  isOpen: boolean;
  onClose: () => void;
  /* Autonomous mode state — passed through from ChatContainer */
...
export function Sidebar({
  ...
  return (
    <>
      ...
      {/* Sidebar panel */}
      <aside
        className={`fixed top-[52px] left-0 bottom-0 z-50 w-[280px] bg-[#09090b] border-r border-white/5 flex flex-col transition-transform duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] lg:translate-x-0 lg:relative lg:top-0 lg:z-auto ${
              ...
              {sessions.slice(0, 20).map((session) => {
                ...
                return (
                  <button
                    ...
                    onClick={() => {
                      onSessionSelect(session, session.analyses[0]);
                      onClose();
                    }}
                  ...
                  >
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className="text-[10px] text-zinc-700 ml-2 shrink-0">
                    ...
                    <div className="flex items-center gap-1.5">
                      {types.map((t) => (
                        <span
                          key={t}
                          className="text-[9px] font-medium uppercase tracking-wider text-zinc-600"
                        >
                          {getTypeLabel(t)}
                        </span>
                      ))}
              ...
              <span className="relative flex h-2.5 w-2.5">
                <span
                  className={`absolute inline-flex h-full w-full animate-ping rounded-full opacity-75 ${
                    autonomousEnabled ? "bg-emerald-400" : "bg-zinc-700"
                  }`}
                />
                <span
                  className={`relative inline-flex h-2.5 w-2.5 rounded-full ${
                    autonomousEnabled ? "bg-emerald-500" : "bg-zinc-600"
                  }`}
                />
              </span>
              <div>
                ...
                <p className="text-[10px] text-zinc-600">
                  {autonomousEnabled ? "Running daily" : "Paused"}
                </p>
              </div>
            </div>
            <button
            ...
            >
              <span
                className={`pointer-events-none inline-block h-4 w-4 transform rounded-full bg-white shadow transition duration-200 ${
                  autonomousEnabled ? "translate-x-4" : "translate-x-0"
                }`}
              />
            </button>
          </div>
